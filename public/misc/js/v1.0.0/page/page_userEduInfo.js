...
require([
    ...
    var eduExpTpl = [
            ...
            '<div class="info-key">',
            '<label><span class="red">*</span>在校时间：</label>',
            '</div>',
            ...
            '<div class="info-key">',
            '<label><span class="red">*</span>学校名称：</label>',
            '</div>',
            '<div class="info-value">',
            ...
            '</div>',
            '<div class="info-value">',
            '<input type="text" class="txt txt-long txt-mid input-prof" maxlength="64"  placeholder="所学专业">',
            '<span class="tips tips-right"></span>',
            '</div>',
            '</div>',
            '<div class="user-info-block clearfix">',
    ...
    var saveUserInfo = function (successInfo) {
        ...
        //提交数据
        util.setAjax(
            inter.getApiUrl().saveUserInfoUrl,
            userInfoData,
            function (json) {
                api.close();
                if (json.status) {
    ...
    var init = function () {
        ...
        //教育经历-经历说明-字数限制
        DetailAPI.initIntroduction($('.text-eduInfo'));
    ...
    var initEduInfo = function () {
        ...
        $(".icon-add-info").on("click", function () {
            DetailAPI.clearInputState($schoolInput);
            DetailAPI.checkDateAll({
                year: $eduYear, yearEnd: $eduYearEnd,
                editSelect: $(".edu-collapse"),
                clear: true,
                err: errStr.timeErr,
        ...
        var editItem = function (elem) {
            ...
            DetailAPI.clearInputState($school);
            check.changeClass(true, $yearEnd, $errTips, '');
            ...
            DetailAPI.clearInputState([$school, $prof]);
            ...
            $btnSave.on("click", function () {
                ...
                //检查是否有错误
                if (DetailAPI.checkEditErr(editModule)) {
                    return
                }
                ;
        ...
        $eduInfoAdd.on('click', function () {
            ...
            //检查是否有错误
            if (DetailAPI.checkEditErr($eduCollapse)) {
                return;
            }
            ;
