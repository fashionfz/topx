...
require([
    ...
    'module/charFormat',
    'module/userDetail'
], function(inter, util, common, prettifyForm, textBox, charFormat, DetailAPI){
    ...
    var saveUserInfo = function(sucCall){
       ...
       /* //服务标签不能为空
        ...
        util.setAjax(inter.getApiUrl().saveResume, userInfoData, function(json){
            ...
            window.opener.refreshResumeGrid();
        },function(){
            api.close();
            $.alert('服务器繁忙，请稍后再试！');
        });
    ...
    var initIntroduction = function(){
        ...
        introduction.on("blur keyup", function(){
            ...
            if (textLen < maxLen && textLen !== 0) {
                ...
                errBox.html('个人说明最多' + maxLen + '字符，现已经超过<span class="num">' + more + '</span>字符');
            }else{
                errBox.html('');
            }
        });
