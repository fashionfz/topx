...
require([
    ...
    // 定义我加入的群组列表构造函数
    var GroupJoinList = function(){
        this.init();
    };

    GroupJoinList.prototype = {
        init : function(){
            var self = this;
            self.inter = inter.getApiUrl().queryJoinGroups;
            self.getGroupJoinCount();
        },
        getGroupJoinCount : function(){
            var self = this,
                page = arguments[0] || 0,
                pageSize = 10;
        ...
        render : function(list){
                ...
                tradeTpl = [
                    '<tr class="#{cls}" data-id="#{groupId}">',
                            ...
                            '<div class="p20 oh">',
                                    ...
                                    '<div class="search-center fl">',
                                        '<div class="name clearfix">',
                                            '<span class="icon #{authority}"></span>',
                        ...
                        '<td class="operate">',
                            ...
                            '<a href="javascript:" title="退出该群" class="btn-delete  ml10"><em class="icon icon-delete"></em></a>',
                        '</td>',
                    '</tr> '
                ].join(''),
                returnStr = [];
            ...
            if(list && list.length>0){
                ...
                returnStr.push('<tr><td align="center"><p class="p20">还没有加入任何群组</p></td></tr>');
            }
            ...
            util.setAjax(
                ...
                function (json) {
                    ...
                    if(json.status == 1){
                        ...
                        self.getGroupJoinCount(self.params.page);
                    }
                },
                function () {
                    tips.close();
