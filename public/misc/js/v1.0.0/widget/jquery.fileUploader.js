...
(function($) {
	...
	$.fn.fileUploader = function(config){
		...
		config = $.extend({}, {
			...
			allowedExtension: 'jpg|jpeg|gif|png', //允许上传的格式
            ...
            limitErr: function(){},//超过限制时执行的回调
            onUpErr: function(){},//上传失败时执行的回调
			onFileChange: function(){},
			onFileRemove: function(){},
			beforeUpload: function(){}, //提交按钮点击后，文件上传前，触发事件
			beforeEachUpload: function(){}, //每个文件上传完成前回调 返回每个form表单
			afterUpload: function(){},
			afterEachUpload: function(){} //每个文件上传完成后回调

		}, config);

        $.fileUploader.count++;

        console.log('count '+$.fileUploader.count);
        //form多文件上传
        ...
        var pxUploadFormInput = pxUploadForm + '-input',
            isLimit = (config.limit == 0) || !!config.limit,
		...
		var px = {
			...
			init: function(){
				...
				px.form.children('.px-input-button').append(
                    '<span style="margin-left: 37px ;" class="fl">'+ config.selectFileLabel +'</span>'
                );
			...
			printForm: function(){
                ...
                console.log('itr '+config.itr)

				var formId = 'pxupload' + config.itr,
					...
					if(isHtml5){
                        if(that.files.length){
                           for(var i=0;i<that.files.length;i++){
                               if(that.files[i].size > config.maxSize){
                                   config.onSizeErr();
                                   return;
                               }
                           }
...
//					$(this).remove();
			...
			$(pxUploadForm).append(
				...
				.append([
					'<img class="loading mt10" src="'+ ued_conf.root +'/images/loading-tl.gif">',
		...
		function uploadQueue() {
			totalForm = $(pxUploadForm + ' form').parent('.upload-data').length;
			if (totalForm > 0) {
